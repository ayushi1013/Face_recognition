...
user_color      = '#000000'
title_webapp    = "Face Detection and Recognition application"
...
html_temp = f"""
            ...
            """
# <img src = "{image_link}" align="right" width=100px ></h1>
...
def main():
    ...
    selected_menu = option_menu(None,
        ['Picture Recognition', 'Real-time Face Recognition', 'Video Recognition', 'View History', 'Add to Database'],
        icons=['camera', 'camera-video', "webcam", 'file-play', "clock-history", 'person-plus'],
        ## icons from website: https://icons.getbootstrap.com/
        menu_icon="cast", default_index=0, orientation="horizontal")

    if selected_menu == 'Picture Recognition':
        selected_button = option_menu(None,['Stop Validation', 'Start Validation'], orientation="horizontal")
        if selected_button == "Start Validation":
    ...
    if selected_menu == 'Real-time Face Recognition':
        selected_tab = option_menu(None,['Same Tab', 'New Tab'], orientation="horizontal")

        if selected_tab == "Same Tab":
    ...
    if selected_menu == 'Video Recognition':
        ...
        if uploaded_file is not None:
            ...
            while cap.isOpened() and not stop_button_pressed:
                ...
                # Check if it's time to process and display the frame
                if int(cap.get(cv2.CAP_PROP_POS_FRAMES)) % frame_skip_interval == 0:
                    # Resize frame for faster processing and display
                    resized_frame = cv2.resize(frame, (0, 0), fx=0.25, fy=0.25)

                    # Convert color from BGR to RGB
                    image_array = cv2.cvtColor(resized_frame, cv2.COLOR_BGR2RGB)
                    ...
                    for idx, (top, right, bottom, left) in enumerate(face_locations):
                        ...
                        cv2.rectangle(image_array, (left, top), (right, bottom), COLOR_DARK, 2)
                        cv2.rectangle(image_array, (left, bottom + 17), (right, bottom), COLOR_DARK, cv2.FILLED)
                        font = cv2.FONT_HERSHEY_DUPLEX
                        cv2.putText(image_array, f"#{idx}", (left + 5, bottom + 12), font, .5, COLOR_WHITE, 1)
                    ...
                    if max_faces > 0:
                        ...
                        if (len(face_idxs) > 0):
                            ...
                            for face_idx in face_idxs:
                                # Getting Region of Interest for that Face
                                roi = rois[face_idx] #image_array[top:bottom, left:right]
                                ...
                                if len(faces) < 1:
                                    ...
                                    if dataframe_new.shape[0] > 0:
                                        ...
                                        # Draw a Rectangle Red box around the face and label it
                                        cv2.rectangle(image_array_copy, (left, top), (right, bottom), color, 2)
                                        cv2.rectangle(image_array_copy, (left, bottom + 17), (right, bottom), color, cv2.FILLED)
                                        font = cv2.FONT_HERSHEY_DUPLEX
                                        cv2.putText(image_array_copy, f"#{dataframe_new.loc[0, 'Name']} {confidence}", (left + 5, bottom + 12), font, .5, COLOR_DARK, 1)

                                        flag_show = True

                    # Display the resulting image
                    #st.image(image_array_copy, channels="RGB", use_column_width=True)
                    frame_placeholder.image(image_array_copy)
        ...
        if(len(face_name) > 1):
            ...
            if pic_option == 'Upload a Picture':
                ...
                img_file_buffer = col3.file_uploader('Upload a Picture',
                                                    type=allowed_image_type, key=unique_key)
                if img_file_buffer is not None:
